...
| `afk-ancdrop.js` | 遠古系裝備的兩個來源:**①金卡怪掉落**、**②該分類已收集滿的製作成品**(**遠古系在上游是「絕版」不是「不存在」**——`anc` 欄位、四階效果 `applyAncStats`、名稱前綴配色、排序、堆疊簽章、廢品標記全在,只有產生那步被關掉:js/07 `rollAffixesNew` 恆回 `anc:false`,全 repo 無任何處寫入 `anc=true`。本支只補「來源」。①條件=`cardDexTier(怪名)>=3`;機率**直接用核心傳進 `rollAffixesNew` 的第一參**(一般 1%/頭目 10%)→上游調祝福率會自動跟上,不必兩邊各記一份;階級四等分。**只包 `rollAffixesNew` 一個點**就涵蓋一般掉落/血盟攻城掉寶/離線結算(離線=1:1 重放核心 killMob)。⚠️ **判斷順序必須是「先確認已金卡才呼叫 lootRng」**——掉落是 committed RNG(吃 `player.lootSeq` 遞增序號),多消耗一個就會位移之後**所有**掉落結果,順序寫反等於光是裝了外掛就改掉所有玩家的掉落序列(已驗:未開金卡 seqPerRoll 維持 1)。⚠️ 怪名要先過 `CARD_DROP_ALIAS`,否則變身鏈最終階(殺生石/安塔瑞斯後續階)查不到卡而安靜失效) | ②**製作**:成品所屬的裝備收集冊分類全收集(js/16 `equipCatComplete`)才開放,機率同樣直接用核心傳進來的 `baseChance`(製作是 10%)。⚠️ **要認「正在產生的那件」而不是 doCraft 的成品 id**——核心 `ensureMaterial` 會在同一次 doCraft 裡遞迴補製中間物(也走 gainItem),只看成品 id 會讓中間物拿成品的分類去判定(判錯,還白吃一個掉落亂數序號);故另包 `gainItem` 記下當下 id,只有「id 就是這次要做的成品」才算數。提示各補一處:裝備收集冊分類頁首、製作面板該配方旁的「✦遠古」標記(包五個製作 NPC 共用的 `craftActionHtml`)。
