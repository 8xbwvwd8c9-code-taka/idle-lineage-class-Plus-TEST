...
| `afk-bmprice.js` | 黑市收購價提示。潘朵拉收購單的命中條件是「系統替指定物品擲的行情價 ≤ 你的出價」,而**成交價就是你自己出的那個數字** —— 遊戲一句都沒講,玩家只能靠掛單試。本支在收購欄下方即時算三件事:必中價(行情價的最大可能值)、你當下出價的每輪命中率、平均要等多久(行情是均勻分布,所以都算得出來)。另開 `window.AFK_BM.itemInfo(id)` 給 afk-dex 的物品詳情標同一個數字(小百科裝備頁走同一支 `itemDetailHTML`,一次涵蓋兩邊)。🚨 **絕不可呼叫 `pandoraBuyOrderPrice`**——它走 `lootRng`,查個價就把 `player.lootSeq` 加一(committed RNG 序號·進存檔受簽章),會位移玩家之後所有掉落;只用不擲骰的 `pandoraBuyOrderPriceProfile` / `pandoraCardPriceRange` 拿區間,最大值即必中價。輪換間隔從面板標題那句「每 10 分鐘輪換 1 件」regex 抓(核心 `PANDORA_SLOT_TICKS` 是頂層 const 拿不到),抓不到退回 10 並由 smoke 回報。名稱完全吻合時會收掉上游的建議下拉——它絕對定位、正好蓋在提示列上,而「手動打完整個名字」正是最需要看到數字的時候 |
