...
>
> 本文是靜態掃描結果（核心 `js/*.js`＋外掛 65 支＋`css/`），嚴重度是「頻率 × 單次工作量 × 背景是否照跑」的推估，**未經真機 profiler 量測**；要動手前先看文末「量測與驗證」。
...
| 已有的 | 位置 | 缺口 |
|---|---|---|
| 省電模式面板六開關（降更新頻率/關動畫/關特效/關傷害數字/關音樂/關音效） | `afk-powersave.js` | 全部**預設關**；`lowfps` 只把 10Hz 節到 8fps（125ms），省不到 20%；`renderStatusEffects` 走 `js/03:699` 直呼、不經 `updateUI`，**不受 lowfps 管**；完全沒碰 CSS 濾鏡與外掛層輪詢 |
...
8. **新選項「關閉光暈與濾鏡特效」**（治 C1/C2/C6/C7，C3 可一併試）：注入一段 `<style>` 覆寫——拔掉 `css/style.css:969` 全域 img filter、鎖定紅光暈、物品光暈動畫群、日誌色字 filter。CSS 覆寫是外掛正規手段（規則 4），對 GPU 是整塊整塊地省，很可能是**手機發熱體感改善最大**的單一選項。
...
10. **`lowfps` 加強**：①把 `renderStatusEffects` 也納入節流（它繞過 `updateUI`，現在管不到，治 B2/B3 的一半）；②追加更狠的檔位（如 250~500ms）——現在 125ms 對 100ms 只省不到 20%。
