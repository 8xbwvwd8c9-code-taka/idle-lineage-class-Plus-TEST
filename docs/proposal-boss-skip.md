...
> **狀態：已實作，但採用的是本文淘汰掉的那個方案**（2026-08-04）—— 見 `afk-bossavoid.js`。
>
> **本文 §1 判斷錯了**：它把「(a) 遇到不打」整條淘汰、選了「(b) 不要出現」，理由是「不鎖定牠也會被牠打」。
> 但上游的迴避頭目根本不是「不鎖定」，是**用瞬移卷軸離場**——實際被淘汰掉的其實是「縮小上游迴避範圍」這個選項，
> 而它才是對的。決定性的理由本文自己寫在 §2.4 卻沒串起來：**走上游 `useItem` 等於免費繼承
> 「哪些地圖不能傳送」的全部守衛**（行動限制／軍王之室／傲慢之塔排名／遺忘之島…），被擋下還不消耗卷軸；
> 而 (b) 必須自己維護一張地圖排除清單，上游每加一個副本就可能漏掉，漏掉的症狀還是安靜的。
>
> 實際做出來的設計：
> - 資料模型 **`{地圖key: [怪id]}`、依存檔位分開**（本文的 (b) 只有一份全域清單，無法「這張圖躲、那張圖打」）
> - 空清單 = 那張圖全部迴避 = 上游今天的行為 → 沒設定過的玩家體驗完全不變
> - 掛點從 `spawnMob` 改成 `autoActions`：把沒挑到的 BOSS 暫時標 `noAutoTeleport`，借上游自己的條件縮小範圍
> - UI 從自動化分頁底部的外掛列，改到**「迴避頭目(瞬移卷軸)」勾選框正下方**，只列目前這張地圖的頭目
>
> 本文仍然有效的部分：§2.1～2.3 的機制與數量級盤點、§5.2 的掉落/任務影響表、§6 的驗收方式。
