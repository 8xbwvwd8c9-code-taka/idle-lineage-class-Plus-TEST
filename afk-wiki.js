...
(function () {
  ...
  function blessRateTableHtml() {
    return wTbl(['來源', '帶「祝福的」機率'], [
      ...
      ['製作／合成', '<b>10%</b>'],
      ['潘朵拉黑市的裝備商品', '<b>1%</b>'],
      ['席琳的世界・一般怪', '<b>3%</b>（1% 的 ×3）'],
      ['瘋狂的席琳世界・一般怪', '<b>5%</b>（1% 的 ×5）'],
      ['席琳的世界・頭目', '<b>20%</b>（固定值，不是 10% 再 ×3）'],
  ...
  var ENHANCE_SECTIONS = [
    ...
    { t: '三種詞綴（屬性／遠古／祝福，可同時各帶一個）', lines: [
      ...
      '<b>遠古系（遠古／永恆／不朽／太初，共四階）</b>：依部位給不同加成，武器／防具／飾品 各列如下（⚠️ <b>現在已經無法取得</b>，只在舊裝備上看得到，詳見下一段）：',
    ...
    { t: '祝福／詛咒怎麼來', lines: [
      '撿到的裝備有機率直接帶「祝福的」，<b>機率看來源</b>（下表）。這個機率只決定「掉出來的那件有沒有祝福」，跟掉不掉東西無關。',
      '取得「對武器／盔甲施法的卷軸」時，各有 <b>1%</b> 機率變成「祝福的」／「詛咒的」卷軸（兩者互斥）。',
      '<b>潘朵拉黑市</b>的裝備是<b>上架那一刻就擲好</b>祝福，買下去只是交付既定結果，讀檔重買不會變。'
    ], html: blessRateTableHtml },
    { t: '詞綴怎麼來', lines: [
      '<b>遠古系（遠古／永恆／不朽／太初）：目前沒有任何取得途徑</b>。打怪、製作、黑市都不會產生它；以前用的「賦予武器／盔甲祝福卷軸」<b>已停用</b>（物品說明就寫著「已無用途」），象牙塔的<b>碧恩現在只賦予屬性</b>。所以身上若有遠古系裝備，那是<b>過去留下來的</b>——上面那張加成表對它們仍然有效。',
