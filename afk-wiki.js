...
(function () {
  ...
  const cfg = window.CUSTOM_CONFIG ?? {};
  const pandora = cfg.PANDORA ?? {};

    out += wCard('🛒 潘朵拉黑市（陳列與收購單）',
      wTbl(['項目', '規則'], [
        ['陳列格數', '<b>' + (pandora.SLOT_COUNT ?? 24) + ' 格</b>'],
        ['輪換速度', '<b>每 ' + (pandora.SLOT_MINUTES ?? 10) + ' 分鐘換掉 1 格</b>'],
        ['單件在架多久', '<b>' + (pandora.KEEP_MINUTES ?? 240) + ' 分鐘</b>'],
