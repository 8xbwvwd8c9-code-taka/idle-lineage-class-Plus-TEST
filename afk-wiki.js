...
(function () {
  ...
  var WEAPON_TRAITS = [
    ...
    { n: '疾病（惡魔王武器）', d: '攻擊時 10% 機率對目標施放「疾病術」，讓目標陷入疾病狀態 30 秒：你的命中 +8（更容易打中）、目標的命中 −4（更容易揮空）。代表武器：惡魔王系列（矛、雙刀、雙手劍、魔杖、弓）。' },
  ...
  var KINGROOM_SECTIONS = [
    { t: '軍王之室是什麼、有哪幾間', lines: [
      ...
      '軍王本身是<b>被動怪</b>（不會主動攻擊你，你先打才會還手），但兩側小怪是主動的。'
    ]},
  ...
  function renderMap() {
    ...
    h += '<div class="m-wiki-desc">「死亡騎士之書」由長老之室的「長老．X」系列怪掉（各 1%）。兩間 BOSS 房：<b>每次 BOSS 復活會再扣 1 個入場道具</b>，扣光就被踢回長老會議廳；挑戰吉爾塔斯時帶著<b>完整的召喚球</b>，中途離開會消耗 1 顆把 BOSS 的傷勢封在原地（下次接著打）。</div>';
    return h;
  }
  ...
  function renderPets() {
    ...
    return note
      + wCard('① 怎麼抓：用誘捕道具，再把那隻動物打死',
          ...
          + wDesc('四隻蜥蜴的強度<b>以同等級的黃金龍為基準</b>再各自乘上倍率：破滅（普攻最高、無視部分傷害減免）、災厄（最耐打，會自己加傷害減免，也最容易被怪盯上）、詛咒（魔法最強，冰雪暴有機率冰凍）、厄運（普攻與魔法都偏高）。四隻的第二招都是<b>必中的持續傷害</b>（灼燒／出血／中毒，每秒固定傷害）。'))
  ...
  var MERC_SKILL_COMMON = [
    ...
    '💀 <b>傭兵召喚出來的東西也會被打死</b>：傭兵的殭屍／精靈／召喚物<b>有自己的血量</b>，會被怪鎖定攻擊、也吃你的全隊治癒；<b>隊伍面板最下方有這些召喚物的血條</b>（戰場上不會畫出來）。被打死或時間到，只要<b>來源角色有勾自動施放</b>、傭兵不在安全區、沒被硬控且 MP 夠，就會<b>自動重召</b>。（你自己迷魅的怪不受影響、仍是無敵的。）'
  ];
  ...
  function renderSunrise() {
    ...
    out += wCard('👹 另外兩隻大頭目',
      ...
      wDesc('<b>' + mobNm('sr_gashadokuro') + '</b>（北之地，Lv.' + mobLv('sr_gashadokuro') + '、HP ' + mobHp('sr_gashadokuro') + '、魔防 300）：招式<b>恐怖的面貌</b>會<b>隨機免疫「近距離／遠距離／魔法」其中一種傷害約 10 秒</b>——被免疫的那類傷害<b>完全打不進去</b>（含觸發法術、雙擊、副手、連射、共鳴、寵物與召喚，連致命的那一擊也會被擋掉）。<b>中毒、灼燒、出血這類持續傷害不受影響，照樣扣血</b>，所以開這招時可以先靠 DoT 磨。另有「<b>枯竭詛咒</b>」讓你中<b>藥水霜化</b>（喝藥回復量減半）。'));
