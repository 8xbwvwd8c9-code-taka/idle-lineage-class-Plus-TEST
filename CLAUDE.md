...
## 🧭 先質疑、再最小化(第一性原理 Phase 0 ＋ ponytail 階梯 — 所有 agent 通用)

動手做任何事之前,過兩道關卡:

**第一關 — 質疑需求本身(Delete First,第一性原理 Phase 0)**
- 所有需求在某種程度上都是蠢的;最蠢的事是優化一個不該存在的東西(Musk)。
- 先試「整個刪掉」:這功能真的需要嗎?它為誰解決什麼問題?
- 刪得掉 → 結束,一句話說明為什麼不做。刪不掉 → 進第二關。
- 想跑完整的 6 階段分析(本質/假設/基石/向上推導/驗證),用 `/first-principles` 技能。

**第二關 — 最小化階梯(ponytail)**,停在第一個成立的台階:

1. **repo 裡已經有了嗎?** 先 grep——重寫「幾行外就有的東西」是最常見的浪費。
2. **標準庫 / 平台原生做得到嗎?** 用原生(`<input type="date">` 勝過 date picker 套件、CSS 勝過 JS、DB 約束勝過應用層檢查)。
3. **已裝的依賴解得掉嗎?** 用。絕不為了「幾行就做得完」的事新增依賴。
4. **一行可以嗎?** 就寫一行。
5. 都不行,才寫「剛好能用」的最小程式碼。

**lite 模式的行為**:照常做出要求的東西,但在同一則回應裡用一行話點出更省的替代方案,由使用者選擇(例:「已加快取。FYI:`lru_cache` 一行就覆蓋,不想養 cache class 的話換這個」)。不要為了推銷最小解而卡住不交付。

規則:
- 不加沒被要求的抽象(只有一個實作的 interface、只生產一個產品的 factory、永遠不變的值卻上 config)——這就是程式層級的「這真的需要嗎」,Phase 0 的延伸。
- 刪除優於新增;無聊優於聰明(聰明是凌晨三點要解的謎)。檔案越少越好、最短可用 diff 勝出——但前提是**先讀懂問題**:改錯地方的最小改動不是懶,是第二個 bug。
- 修 bug = 修根因,不是修症狀:動手前先 grep 要動的函式的所有呼叫端,在共用處修一次,比在每個呼叫端各貼一道防護更小、也更不會漏掉兄弟呼叫端。
- 砍到真實角落(全域鎖、O(n²) 掃描、naive heuristic)時,用 `ponytail:` 註解標明天花板與升級路徑。
- 非一行能寫完的邏輯,留**一個**能跑的最小檢查(assert 自檢或一支小 test;不要框架、不要 fixture)。一行就能寫完的不用。

**永遠不省的**:trust boundary 的輸入驗證、防止資料遺失的錯誤處理、安全性、無障礙、以及使用者明確要求的事。質疑的是需求與方案,不是理解問題——先讀完整個流程再動手。

出處:first-principles Phase 0(danyuchn/first-principles-skill)＋ ponytail(DietrichGebert/ponytail),採 lite 模式。
