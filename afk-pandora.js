// ===== 潘朵拉商城與賭場（afk-pandora）=====
// 由「討論版娛樂 MOD 合輯」改寫成加掛版外掛：鬥技賭場 + 骰子賭場 + 潘朵拉商城與龍鑽。
// 三個功能各自獨立、互不依賴；各自包在獨立 try/catch，任何一個失敗不影響其他。
// 入口按鈕統一插進自動化分頁「🔌 外掛」列（#m-afk-navrow，比照 afk-dograce/afk-training）。
// 自訂圖片統一放 custom-assets/pandora/（assets/、public/ 會被 CI rsync --delete 清掉，不可放）。
(function () {
    'use strict';
    if (window.AFK_TOGGLES && !AFK_TOGGLES.enabled('pandora')) return;   // 🎚️ 外掛開關

    // 自訂圖片統一由此管理（不要散落硬編碼）
    var IMG_BASE = 'custom-assets/pandora/';

    // 共用入口列：自動化分頁「🔌 外掛」列（#m-afk-navrow）加一顆全寬按鈕
    function injectNavButton(id, label, title, onClick) {
        ...
        var hdr = row.querySelector('.text-amber-400');
    ...
    // 重試注入（自動化面板可能晚於外掛才建立）
    function retryInject(fn) {
        var tries = 0;
        (function tryInject() {
            if (fn()) return;
            if (++tries < 40) setTimeout(tryInject, 500);
            else console.warn('[AFK-pandora] 找不到 tab-automation，入口未注入');
        })();
    }
...
`;
    ...
    // ---------- 入口：自動化分頁「🔌 外掛」列加一顆鈕（比照 afk-dograce/afk-training） ----------
    function init() {
        retryInject(function () { return injectNavButton('m-afk-nav-arena', '⚔️ 鬥技賭場', '鬥技場：下注觀戰，兩位角色隨機對決', openArenaVis); });
    }
    ...
    } catch (e) { console.warn('[AFK-pandora] 鬥技賭場初始化失敗：', e); }
...
(function () {
    ...
    const CANVAS_W = 800, CANVAS_H = 450;
    const ARENA_BG = IMG_BASE + '賭桌.jpg';
...
`;
    ...
    // ---------- 入口：自動化分頁「🔌 外掛」列加一顆鈕（比照 afk-dograce/afk-training） ----------
    function init() {
        retryInject(function () { return injectNavButton('m-afk-nav-casino2', '🎲 骰子賭場', '天堂賭場（重製版）：搶莊、壓大壓小', openCasino2); });
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init);
    else init();
    ...
    } catch (e) { console.warn('[AFK-pandora] 骰子賭場初始化失敗：', e); }
...
(function () {
    ...
    const CANVAS_W = 800, CANVAS_H = 450;
    const SHOP_BG = IMG_BASE + '潘朵拉商城.png';
...
`;
    ...
    // ---------- 入口：自動化分頁「🔌 外掛」列加一顆鈕（比照 afk-dograce/afk-training） ----------
    function init() {
        retryInject(function () { return injectNavButton('m-afk-nav-shop', '🛒 潘朵拉商城', '潘朵拉商城：用龍鑽買遺物', openPandoraShop); });
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init);
    else init();

    // ---------- 龍之鑽石掉落（monkey-patch，Fail Soft：上方已確認 killMob 存在才進得來） ----------
    ...
    } catch (e) { console.warn('[AFK-pandora] 潘朵拉商城初始化失敗：', e); }
})();
